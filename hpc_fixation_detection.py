...
class HPCFixationDetection:
    ...
    def submit_job_array(self, job_file_path):
        try:
            ...
            subprocess.run(
                f'module load dSQ; dsq --job-file {job_file_path} --batch-file {job_script_path} -o {self.job_script_out_dir} --status-dir {self.job_script_out_dir} --partition psych_day --cpus-per-task 8 --mem-per-cpu 4096 -t 4:00:00 --mail-type FAIL',
